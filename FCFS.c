...
int main()
{
	...
	while (1)
	{
		...
		for (i = 1; i <= num_proc; i++) // 윗줄
		{
			...
			n = i - 1;
			if (at[i-1] > ct[n])          // 프로세스 사이에 빈 시간이 있을경우에 출력되는 간트차트의 빈 공간의 윗줄 "-" 
			{
				
				printf(" -");
				for (k = 0; k < (at[i-1] - ct[n]); k++)
				{
					printf("-");
				}
			}
		...
		for (i = 1; i <= num_proc; i++) // 가운데 줄
		{
			...
			n = i - 1;
			if (at[i-1] > ct[n])          // 프로세스 사이에 빈 시간이 있을경우에 출력되는 간트차트의 빈 공간의 가운데 줄의 공백
			{
				printf("| ");
				for (k = 0; k < (at[i-1] - ct[n]); k++)
				{
		...
		for (i = 1; i <= num_proc; i++) // 아랫줄
		{
			...
			n = i - 1;
			if (at[i-1] > ct[n])          // 프로세스 사이에 빈 시간이 있을경우에 출력되는 간트차트의 빈 공간의 아랫줄 "-"
			{
				printf(" -");
				for (k = 0; k < (at[i-1] - ct[n]); k++)
				{
