...
int main()
{
	...
	while (1)//무한반복:탈출
    {
		...
		g_p=(int*)realloc(g_p,sizeof(int)*num_proc);
		g_et=(int*)realloc(g_et,sizeof(int)*num_proc);
		g_bt=(int*)realloc(g_bt,sizeof(int)*num_proc);
		...
		while(N>0){ //종료조건: line136 
			...
			for(i=0;i<N;i++){//실행이 끝나지 않은 프로세스들중 가장 남음시간이 짧은 프로세스의 index 찾기
